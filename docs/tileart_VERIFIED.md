...
| **Effects** (SUB_9_8)         | `u8 effect_count` + `count × 32-byte` effect body | Visual effects. **`u8 count` then `count` fixed 32-byte bodies** — verified by exact tail length (`len == 1 + 32·count`): count 0 (39,803 tiles, **99.0 %**), 1 (291), 2 (16). The 32-byte body is `u32 type(=0)`, `u16 effect-id`, then param floats (e.g. 0.1 / 2.0). *(An earlier note claimed variable "opcodes 12/88/96" — that was **parser drift** on 93 tiles with complex texture-groups, not real multi-opcode effects.)* Stored opaque as `EffectsTail` in C#. |
