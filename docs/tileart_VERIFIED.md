...
| 0x29   | u32      | Always 0                    | ✅       |                                                        |
| 0x2D   | f32      | **Light param 0** (`Lights[0]`) | ✅   | Light-source parameter. Non-zero on **1,237 tiles, 93 % of which have the CC `LightSource` flag** (and 86 % of CC LightSource tiles have it). Lit tiles are openings — `window`, `arrow loop`. Signed, range -5.0..2.0; varies by tile **orientation** (e.g. left/right-facing windows get ±1.35/1.75) → looks like a **lateral light-source offset/direction**. (UOReader only labelled it `LightRelated`.) |
| 0x31   | f32      | **Light param 1** (`Lights[1]`) | ✅   | Second light param. Range -11.5..0; clusters per tile family (windows `-3.0`, arrow loops `-4.0`) → looks like a **vertical/Z light-source offset**. Inferred; the precise lighting model isn't in any spec. |
...
| **Effects** (SUB_9_8)         | `u8 count` + `count ×` `{u32 EffectID, sub, opcode FUNCTION}` | Visual effects. **99.0 % of statics have `count == 0`** (empty). Per UOReader's spec each effect is `u32 EffectID`, a `4_6_30_Sub` struct, then a **FUNCTION dispatched by an opcode** — opcodes `00,01,02,07,10,11,12,15,16,17` each have their **own body layout, undecoded even by UOReader** (`Unk_3/Unk_4/…`). Empirically the **common opcode (≈ count 1–2) parses as a 32-byte body** (`u32`, `u16 effect-id`, param floats) — 307 tiles; **~93 tiles use other opcodes with variable bodies** my fixed-size parser can't follow (these are *real* multi-opcode effects, not parser drift — an earlier draft wrongly dismissed them). Stored opaque as `EffectsTail` in C#; full per-opcode decode is open (and not in any spec). |
